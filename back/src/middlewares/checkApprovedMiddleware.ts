...
export async function checkApprovedMiddleware(
  ...
  try {
    ...
    if (!decodedToken.approved) {
      res.status(403).json({ error: "El usuario no está aprobado para acceder a este recurso." });
      return;
    }
    ...
    res.status(401).json({ error: "Cookie de sesión inválida o error en la verificación." });
  }
}
