...
export async function assignAdminRoleController(req: Request, res: Response): Promise<void> {
  const { uid } = req.body;
  ...
  try {
    ...
    res.status(200).json({ message: "El usuario ha sido asignado como admin correctamente." });
    return;
  } catch (error: any) {
